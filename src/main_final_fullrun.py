...
from get_metrics_final_fullrun import compute_dset_fragment_scores
import pickle
...
def main(args):
    ...
    if args.no_chkpt: print("\nUsing final (likely overfit) version of network for outputs because no checkpoints were saved")
        ...
        #checkpoint_path ='../data/checkpoints/11-27_12:32:24.pt'
    ...
    #train_dl, val_dl, test_dl = data_loader.get_split_dls(json_data['dataset'],splits,batch_size=1,shuffle=True,i3d=args.i3d,video_data_dir=video_data_dir)
    ...
    if args.VG:
        ...
        if args.dataset == 'MSVD':
            ...
            #Change directory and dataset in build_LR_dataset()
            build_LR_dataset(6, train_dl, encoder, dataset_dict, attribute_stats, relationship_stats, sub_count_c, sub_count_r, args.i3d, 'train')
            build_LR_dataset(6, val_dl, encoder, dataset_dict, attribute_stats, relationship_stats, sub_count_c, sub_count_r, args.i3d, 'val')
            build_LR_dataset(6, test_dl, encoder, dataset_dict, attribute_stats, relationship_stats, sub_count_c, sub_count_r, args.i3d, 'test')

            #LR_datapath = '../data/MSVD/train_LR/' 
            LR_datapath = '../data/train_LR/MSVD/epoch6'
            model = train_LR(LR_datapath)
            coeff = model.coef_
            print("Model Weights :",coeff)
        else:
            #change directory and dataset for built_LR_dataset
            build_LR_dataset(6, train_dl, encoder, dataset_dict, attribute_stats, relationship_stats, sub_count_c, sub_count_r, args.i3d, 'train')
            build_LR_dataset(6, val_dl, encoder, dataset_dict, attribute_stats, relationship_stats, sub_count_c, sub_count_r, args.i3d, 'val')
            build_LR_dataset(6, test_dl, encoder, dataset_dict, attribute_stats, relationship_stats, sub_count_c, sub_count_r, args.i3d, 'test')
            LR_datapath = '../data/train_LR/MSRVTT/epoch6' 
            model = train_LR(LR_datapath)
            coeff = model.coef_
            print("Model Weights :",coeff)
