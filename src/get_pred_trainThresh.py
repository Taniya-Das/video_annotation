...
def topK_classifications(ind_multiclassifications, class_multiclassifications, rel_multiclassifications):
    ...
    #a = np.random.rand((285,285))
    #b = np.random.rand(150)
    #c = np.expand_dims(a,2) * b
...
def inference(video_ids, encodings, ind_multiclassifications, class_multiclassifications, rel_multiclassifications, dataset_dict, fragment_name, threshold):
    ...
    if len(atoms)==0:
        ...
        if fragment_name == 'train':
            ...
            for key, value in mlp_value_dict.items():
                if (value>threshold):best_annotation.append(key)
            tp, fp, fn, tn, best_f1, best_acc = compute_f1_for_thresh(best_annotation, atoms, lcwa)
            print("tp: {}, fp: {}, fn: {}, tn: {}, f1: {}".format(tp,fp,fn,tn,best_f1))
...
def compute_probs_for_dataset(dl,encoder,multiclassifier,multiclassifier_class,multiclassifier_rel,dataset_dict,use_i3d,fragment_name,threshold):
    ...
    return pos_classifications, neg_classifications, pos_classifications_class, neg_classifications_class, pos_classifications_rel, neg_classifications_rel, pos_predictions, neg_predictions, perfects, total_acc, total_f1, threshold
