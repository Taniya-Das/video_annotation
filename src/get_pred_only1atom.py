...
def pos_neg_accuracy(tp,tn,fp,fn):
    positive_accuracy = int(tp)/(int(tp)+int(fn))
    negative_accuracy = int(tn)/(int(tn)+int(fp))
    return positive_accuracy, negative_accuracy

def inference(video_ids, encodings, ind_multiclassifications, class_multiclassifications, rel_multiclassifications, dataset_dict, fragment_name, threshold):
    ...
    if len(atoms)==0:
        ...
        tp, fp, fn, tn, best_f1, best_acc = compute_f1_for_thresh(best_annotation, atoms, lcwa)
        pos_acc, neg_acc = pos_neg_accuracy(tp,tn,fp,fn)
        print("positive_accuracy: {}, negative_accuracy: {}, f1: {}, acc: {}".format(pos_acc,neg_acc,best_f1,best_acc))
...
def compute_probs_for_dataset(dl,encoder,multiclassifier,multiclassifier_class,multiclassifier_rel,dataset_dict,use_i3d,fragment_name,threshold):
    ...
    for d in dl:
        ...
        if fragment_name == 'train':
            annotations_by_id = inference(video_ids, encodings, multiclassif, multiclassif_class, multiclassif_rel, dataset_dict, fragment_name, threshold)
            all_thresh += [vid['thresh'] for vid in annotations_by_id.values()]
        else:
            annotations_by_id = inference(video_ids, encodings, multiclassif, multiclassif_class, multiclassif_rel, dataset_dict, fragment_name, threshold)
        ...
        all_pos_acc += [vid['pos_acc'] for vid in annotations_by_id.values()] 
        all_neg_acc += [vid['neg_acc'] for vid in annotations_by_id.values()] 
    ...
    total_pos_acc = np.array([x for x in all_pos_acc if x!=-1]).mean()
    total_neg_acc = np.array([x for x in all_neg_acc if x!=-1]).mean()

    if fragment_name == 'train':
        total_thresh = np.array([x for x in all_thresh if x!=-1]).mean()
        threshold = total_thresh
    #breakpoint()

    return pos_classifications, neg_classifications, pos_classifications_class, neg_classifications_class, pos_classifications_rel, neg_classifications_rel, pos_predictions, neg_predictions, perfects, total_acc, total_f1, threshold, total_pos_acc, total_neg_acc
