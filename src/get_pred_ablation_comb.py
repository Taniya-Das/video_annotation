"""
Get predictions for ablation on combining tenchiques. The topk values are replaced by  simple thresholding method
with threshold = 0.5
Can be used by changing the get_pred_final.py to get_pred_ablation_comb.py in get_metric_final.py
"""
...
def mlp_value_dict_compute_threshold(inds_to_use, class_to_use, rel_to_use, encoding):
    mlp_value_dict = {}
    with torch.no_grad():
        for subj_id, subj_vector in inds_to_use:
            for class_id, class_mlp in class_to_use:
                ...
                mlp_value_dict.update({(class_id, subj_id):numpyify(F.sigmoid(class_mlp(context_embedding)))})
        for obj_id, obj_vector in inds_to_use:
            ...
            for rel_id, rel_mlp in rel_to_use:
                ...
                mlp_value_dict.update({(rel_id,subj_id,obj_id):numpyify(F.sigmoid(rel_mlp(context_embedding)))})
...
def inference(video_ids, encodings, ind_multiclassifications, class_multiclassifications, rel_multiclassifications, dataset_dict, dataset, threshold, threshold_VG, fragment_name, attribute_stats, relationship_stats, sub_count_c, sub_count_r, model, VG):
    ...
    if len(atoms)==0:
        ...
        for video_id, encoding, vid_ind_c, vid_class_c, vid_rel_c in zip(video_ids,encodings,ind_multiclassifications, class_multiclassifications, rel_multiclassifications):
            ...
            inds_to_use = [list(ind_dict.items())[i] for i,c in enumerate(vid_ind_c) if c>-1.7]
            class_to_use = [list(pred_mlps['classes'].items())[i] for i,c in enumerate(vid_class_c) if c>-1.7]
            rel_to_use = [list(pred_mlps['relations'].items())[i] for i,c in enumerate(vid_rel_c) if c>-1.7]

            mlp_value_dict = mlp_value_dict_compute_threshold(inds_to_use, class_to_use, rel_to_use, encoding)
