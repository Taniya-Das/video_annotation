...
def inference(video_ids, encodings, ind_multiclassifications, class_multiclassifications, rel_multiclassifications, dataset_dict, attribute_stats, relationship_stats):
    ...
    if len(atoms)==0:
        ...
        with torch.no_grad():
            for i in range(len(index_class)):
                ...
                mlp_value_dict.update({(class_id, sub_id):numpyify(class_mlp(context_embedding))[0]+attribute_stats.get((class_id, sub_id),0)})
                if attribute_stats.get((class_id, sub_id),0): count_attri = count_attri+1
            ...
            for i in range(len(index_rel)):
                ...
                mlp_value_dict.update({(rel_id,sub_id,obj_id):numpyify(rel_mlp(context_embedding))[0]+relationship_stats.get((rel_id,sub_id,obj_id),0)})
                if relationship_stats.get((rel_id,sub_id,obj_id),0): count_rel = count_rel + 1
    ...
    annotations_by_id[video_ids[0]] = {'annotation': best_annotation,
                                       'acc': best_acc,
                                       'f1': best_f1}
    print("Statistics Count: ", count_attri, count_rel)                                   
...
def compute_probs_for_dataset(dl,encoder,multiclassifier,multiclassifier_class,multiclassifier_rel,dataset_dict,use_i3d):
    ...
    for d in dl:
        ...
        annotations_by_id = inference(video_ids, encodings, multiclassif, multiclassif_class, multiclassif_rel, dataset_dict, attribute_stats, relationship_stats)
