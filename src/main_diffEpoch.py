...
from get_metrics_diffEpoch import compute_dset_fragment_scores
import pickle
...
def main(args):
    ...
    checkpoint_path_full ='../data/checkpoints/MSVD_full_.pt'
    checkpoint_full = torch.load(checkpoint_path_full)
    encoder = checkpoint_full['encoder']
    multiclassifier = checkpoint_full['multiclassifier']
    multiclassifier_class = checkpoint_full['multiclassifier_class']
    multiclassifier_rel = checkpoint_full['multiclassifier_rel']
    dataset_dict['ind_dict'] = checkpoint_full['ind_dict']
    ...
    #params_list = [mlp.parameters() for mlp in mlp_dict['classes'].values()] + [mlp.parameters() for mlp in mlp_dict['relations'].values()]
    ...
    with open(summary_filename, 'w') as summary_file:
        ...
        summary_file.write('\n\n')
        summary_file.write(f'With KB                         Without KB\n')
        summary_file.write(f'train threshold: {train_threshold}          {0.45}\n\n')

        for epoch in [0,1,2,5,7,10,15,20,25,30,40,50,60]:
            print("Training Predicate MLP, epoch: ",epoch)
            #train.train_onlyMLP(args, epoch, encoder, dataset_dict, train_dl=train_dl, val_dl=val_dl, optimizer=optimizer, exp_name=exp_name, device=args.device, train=True)
            train.train(args, encoder_unt, multiclassifier_unt, multiclassifier_class_unt, multiclassifier_rel_unt, dataset_dict_unt, train_dl=train_dl, val_dl=val_dl, optimizer=optimizer, exp_name=exp_name, device=args.device, train=True)
            if epoch > 0:
                checkpoint_path = os.path.join(args.data_dir,'checkpoints/{}.pt'.format(exp_name))
                checkpoint = torch.load(checkpoint_path)
                dataset_dict['mlp_dict'] = checkpoint['mlp_dict']

            build_LR_dataset(epoch, train_dl, encoder, dataset_dict, attribute_stats, relationship_stats, sub_count_c, sub_count_r, args.i3d, 'train')
            build_LR_dataset(epoch, val_dl, encoder, dataset_dict, attribute_stats, relationship_stats, sub_count_c, sub_count_r, args.i3d, 'val')
            build_LR_dataset(epoch, test_dl, encoder, dataset_dict, attribute_stats, relationship_stats, sub_count_c, sub_count_r, args.i3d, 'test')

            t1 = time()
            LR_datapath = '../data/train_LR/epoch' + str(epoch)
            model = train_LR(LR_datapath)
            coeff = model.coef_
            print("Model Weights :",coeff)
            print("Time to train LR: ",{asMinutes(time()-t1)})
            
            
            print("Test With and Without KB")
            t1 = time()
            test_acc, test_f1, test_acc_2, test_f1_2 = compute_dset_fragment_scores(test_dl,encoder,multiclassifier,multiclassifier_class,multiclassifier_rel,dataset_dict,'test',args, train_threshold, attribute_stats, relationship_stats, sub_count_c, sub_count_r, model, withKB = True)
            print("Test Eval Time:",{asMinutes(time()-t1)})
            print(test_acc, test_f1, test_acc_2, test_f1_2)

            summary_file.write(f'Epoch: {epoch}\n')
            summary_file.write(f'LR weights [pred-MLP, KB]: {coeff}\n')
            summary_file.write(f'test acc: {test_acc}, \t{test_acc_2}\n')
            summary_file.write(f'test f1: {test_f1}, \t{test_f1_2}\n')
            summary_file.write(f'test time: {asMinutes(time()-t1)}\n')
